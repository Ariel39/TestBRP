...
function SWEP:PrimaryAttack(bodyPart)
    ...
    if not self.minigameScore then
        ...
        local targetBodyPart = bodyPart or easzy.bodydamage.GetMostInjuredBodyPart(owner, "bleeding")
        if not targetBodyPart then return end
        
        -- Solicitar minijuego al cliente
        easzy.bodydamage.RequestMinigame(owner, "bandage", self:GetClass(), targetBodyPart, owner)
        return
    end

    -- Plays the sound
    owner:EmitSound("easzy/ez_body_damage/bandage.mp3")
    ...
    local function Use(ply)
        ...
        easzy.bodydamage.TreatBleeding(ply, targetBodyPart)
        self:UseAmmo()
...
function SWEP:SecondaryAttack()
    ...
    -- Verify that there is something to treat
    local bodyPart = easzy.bodydamage.GetMostInjuredBodyPart(ent, "bleeding")
    if not bodyPart then return end

    -- Solicitar minijuego
    easzy.bodydamage.RequestMinigame(owner, "bandage", self:GetClass(), bodyPart, ent)
end